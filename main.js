...
;(function(window) {
	...
	DOM.menu = {
		'design' : {
			'wrapper': document.querySelector('.menu1'),
			'items': document.querySelector('.menu1').firstElementChild.querySelectorAll('.menu__inner a')
		},
		'code' : {
	...
	//	DOM.switchModeCtrls.design.addEventListener('click', switchMode);
		...
		DOM.body.addEventListener('mouseenter', contactMouseEnterEvFn);
		DOM.contact.el.addEventListener('mouseleave', contactMouseLeaveEvFn);
		DOM.title.el.addEventListener('mouseleave', contactMouseLeaveEvFn);
